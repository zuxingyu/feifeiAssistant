...
void WifiConfigurationAp::StartWebServer()
{
    ...
    httpd_uri_t form_submit = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            // 获取当前对象
            auto *this_ = static_cast<WifiConfigurationAp *>(req->user_ctx);
            // Save credentials without connecting (connection happens later via /api/finish)
    ...
    // ===== Custom setup: schedule, weather, config storage =====
    // All config steps are now in a single-page app served at /

    // -- GET /api/config/status --
    httpd_uri_t api_status = {.uri="/api/config/status",.method=HTTP_GET,.handler=[](httpd_req_t*r)->esp_err_t{
        nvs_handle_t n; std::string sk, wk, wc;
        auto gs=[&](const char*key){if(nvs_open("setup",NVS_READONLY,&n)!=ESP_OK)return std::string();char b[128]={};size_t z=sizeof(b);esp_err_t e=nvs_get_str(n,key,b,&z);nvs_close(n);return e==ESP_OK?std::string(b):std::string();};
        auto gb=[&](const char*key){if(nvs_open("setup",NVS_READONLY,&n)!=ESP_OK)return std::string();char b[4096]={};size_t z=sizeof(b);esp_err_t e=nvs_get_str(n,key,b,&z);nvs_close(n);return e==ESP_OK?std::string(b):std::string();};
        sk=gb("schedule");wk=gs("weather_secret");wc=gs("weather_city");
        cJSON*o=cJSON_CreateObject();
        cJSON_AddBoolToObject(o,"schedule_configured",!sk.empty());
        cJSON_AddBoolToObject(o,"weather_configured",!wk.empty());
        cJSON_AddStringToObject(o,"weather_key",wk.c_str());
        cJSON_AddStringToObject(o,"weather_city",wc.c_str());
        if(!sk.empty()){cJSON*j=cJSON_Parse(sk.c_str());if(j){cJSON*ss=cJSON_GetObjectItem(j,"semester_start");if(cJSON_IsString(ss))cJSON_AddStringToObject(o,"semester_start",ss->valuestring);cJSON*se=cJSON_GetObjectItem(j,"semester_end");if(cJSON_IsString(se))cJSON_AddStringToObject(o,"semester_end",se->valuestring);cJSON*in=cJSON_GetObjectItem(j,"schedule");if(in){cJSON*si=cJSON_GetObjectItem(in,"single");if(si)cJSON_AddItemToObject(o,"schedule_single",cJSON_Duplicate(si,1));cJSON*du=cJSON_GetObjectItem(in,"dual");if(du)cJSON_AddItemToObject(o,"schedule_double",cJSON_Duplicate(du,1));}cJSON_Delete(j);}}
        char*js=cJSON_PrintUnformatted(o); httpd_resp_set_type(r,"application/json");httpd_resp_set_hdr(r,"Connection","close");httpd_resp_send(r,js,HTTPD_RESP_USE_STRLEN); cJSON_free(js); cJSON_Delete(o); return ESP_OK;
    },.user_ctx=nullptr};
    ESP_ERROR_CHECK(httpd_register_uri_handler(server_, &api_status));

    // -- POST /api/schedule --
    httpd_uri_t api_sched = {.uri="/api/schedule",.method=HTTP_POST,.handler=[](httpd_req_t*r)->esp_err_t{
        char* buf=(char*)malloc(r->content_len+1);
        if(!buf){httpd_resp_set_type(r,"application/json");httpd_resp_set_hdr(r,"Connection","close");httpd_resp_send(r,"{\"success\":false}",HTTPD_RESP_USE_STRLEN);return ESP_OK;}
        int ret=httpd_req_recv(r,buf,r->content_len);
        if(ret<=0){free(buf);httpd_resp_set_type(r,"application/json");httpd_resp_set_hdr(r,"Connection","close");httpd_resp_send(r,"{\"success\":false}",HTTPD_RESP_USE_STRLEN);return ESP_OK;}
        buf[ret]=0;
        // Copy buffer for async NVS write before sending response
        char* saved=(char*)malloc(ret+1);
        if(saved)memcpy(saved,buf,ret+1);
        free(buf);
        ...
        if(saved){
            xTaskCreate([](void* data){
                char* raw=(char*)data;
                cJSON*j=cJSON_Parse(raw);if(!j){free(raw);vTaskDelete(NULL);return;}
                cJSON*s=cJSON_CreateObject();
                cJSON*ss=cJSON_GetObjectItem(j,"semester_start");if(cJSON_IsString(ss))cJSON_AddStringToObject(s,"semester_start",ss->valuestring);
                cJSON*se=cJSON_GetObjectItem(j,"semester_end");if(cJSON_IsString(se))cJSON_AddStringToObject(s,"semester_end",se->valuestring);
                cJSON_AddStringToObject(s,"week_type","single");
                // Convert array format to key-value format expected by ParseScheduleJson
                // JS sends: {"day0":["语文","",...], "day1":[...], ...}
                // ParseScheduleJson expects: {"mon_1":"语文", "mon_2":"数学", ...}
                static const char* day_keys[] = {"mon","tue","wed","thu","fri"};
                auto convert_week = [&](cJSON* src_arr, const char* week_name) {
                    cJSON* out = cJSON_CreateObject();
                    if(cJSON_IsObject(src_arr)){
                        for(int d=0;d<5;d++){
                            char day_key[8];snprintf(day_key,sizeof(day_key),"day%d",d);
                            cJSON* day_arr = cJSON_GetObjectItem(src_arr,day_key);
                            if(cJSON_IsArray(day_arr)){
                                for(int p=0;p<8&&p<cJSON_GetArraySize(day_arr);p++){
                                    cJSON* item = cJSON_GetArrayItem(day_arr,p);
                                    if(cJSON_IsString(item)&&item->valuestring&&item->valuestring[0]){
                                        char k[16];snprintf(k,sizeof(k),"%s_%d",day_keys[d],p+1);
                                        cJSON_AddStringToObject(out,k,item->valuestring);
                                    }
                                }
                            }
                        }
                ...
                cJSON*si=cJSON_GetObjectItem(j,"schedule_single");
                cJSON_AddItemToObject(in,"single",convert_week(si,"single"));
                cJSON*du=cJSON_GetObjectItem(j,"schedule_double");
                if(cJSON_IsObject(du))cJSON_AddItemToObject(in,"dual",convert_week(du,"dual"));
                else{cJSON*dup=convert_week(si,"dual");cJSON_AddItemToObject(in,"dual",dup);}
                cJSON_AddItemToObject(s,"schedule",in);
                char*js=cJSON_PrintUnformatted(s);
                nvs_handle_t nv;
                if(nvs_open("setup",NVS_READWRITE,&nv)==ESP_OK){
                    nvs_set_str(nv,"schedule",js);nvs_commit(nv);nvs_close(nv);
                    ESP_LOGI(TAG,"Schedule saved to NVS, len=%d",(int)strlen(js));
                }
                cJSON_free(js);cJSON_Delete(s);cJSON_Delete(j);free(raw);
                vTaskDelete(NULL);
            },"sched_save",8192,saved,5,NULL);
        }
        return ESP_OK;
    },.user_ctx=nullptr};
    ESP_ERROR_CHECK(httpd_register_uri_handler(server_, &api_sched));

    // -- POST /api/weather --
    httpd_uri_t api_wx = {.uri="/api/weather",.method=HTTP_POST,.handler=[](httpd_req_t*r)->esp_err_t{
        char* buf=(char*)malloc(r->content_len+1);
        if(!buf){httpd_resp_set_type(r,"application/json");httpd_resp_set_hdr(r,"Connection","close");httpd_resp_send(r,"{\"success\":false}",HTTPD_RESP_USE_STRLEN);return ESP_OK;}
        int ret=httpd_req_recv(r,buf,r->content_len);
        if(ret<=0){free(buf);httpd_resp_set_type(r,"application/json");httpd_resp_set_hdr(r,"Connection","close");httpd_resp_send(r,"{\"success\":false}",HTTPD_RESP_USE_STRLEN);return ESP_OK;}
        buf[ret]=0;
        char* saved=strdup(buf);free(buf);
        httpd_resp_set_type(r,"application/json");httpd_resp_set_hdr(r,"Connection","close");
        httpd_resp_send(r,"{\"success\":true}",HTTPD_RESP_USE_STRLEN);
        if(saved){
            xTaskCreate([](void* data){
                ...
                nvs_handle_t nv;
                if(nvs_open("setup",NVS_READWRITE,&nv)==ESP_OK){
                    cJSON*k=cJSON_GetObjectItem(j,"api_key");if(cJSON_IsString(k)&&strlen(k->valuestring)>0)nvs_set_str(nv,"weather_secret",k->valuestring);
                    cJSON*c=cJSON_GetObjectItem(j,"city");if(cJSON_IsString(c)&&strlen(c->valuestring)>0)nvs_set_str(nv,"weather_city",c->valuestring);
                    nvs_commit(nv);nvs_close(nv);
                    ESP_LOGI(TAG,"Weather config saved");
                }
                cJSON_Delete(j);free(raw);
                vTaskDelete(NULL);
            },"wx_save",8192,saved,5,NULL);
        }
        return ESP_OK;
    },.user_ctx=nullptr};
    ESP_ERROR_CHECK(httpd_register_uri_handler(server_, &api_wx));

    // -- POST /api/wifi (save only, no connect) --
    httpd_uri_t api_wifi = {.uri="/api/wifi",.method=HTTP_POST,.handler=[](httpd_req_t*r)->esp_err_t{
        char b[1024]={};int ret=httpd_req_recv(r,b,sizeof(b)-1);if(ret<=0){httpd_resp_set_type(r,"application/json");httpd_resp_set_hdr(r,"Connection","close");httpd_resp_send(r,"{\"success\":false}",HTTPD_RESP_USE_STRLEN);return ESP_OK;}
        b[ret]=0;cJSON*j=cJSON_Parse(b);if(!j){httpd_resp_set_type(r,"application/json");httpd_resp_set_hdr(r,"Connection","close");httpd_resp_send(r,"{\"success\":false}",HTTPD_RESP_USE_STRLEN);return ESP_OK;}
        cJSON*si=cJSON_GetObjectItemCaseSensitive(j,"ssid");
        cJSON*pw=cJSON_GetObjectItemCaseSensitive(j,"password");
        if(cJSON_IsString(si)&&si->valuestring){auto*self=static_cast<WifiConfigurationAp*>(r->user_ctx);self->Save(si->valuestring,cJSON_IsString(pw)?pw->valuestring:"");}
        cJSON_Delete(j);{httpd_resp_set_type(r,"application/json");httpd_resp_set_hdr(r,"Connection","close");httpd_resp_send(r,"{\"success\":true}",HTTPD_RESP_USE_STRLEN);return ESP_OK;}
    },.user_ctx=this};
    ESP_ERROR_CHECK(httpd_register_uri_handler(server_, &api_wifi));
